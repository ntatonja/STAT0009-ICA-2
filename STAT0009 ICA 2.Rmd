...
We can use Sequential Monte Carlo methods and Sequential Importance sampling with resampling to respond the problem of weight degeneracy. Thanks to resampling, the particles with low weights are removed and those with high weights are replicated - we achieve the set of uniformly weighted particles that are approx. distributed according to the posterior $p(\phi_{0:t}|y_{1:t})$. Sequentially, at the new time points, the new hidden states are added from a conditional importance distribution -  $q_{t+1}(\phi_{t+1}^{(i)}| \phi_{0:t}^{(i)})$. A generic particle filter algorithm is as follows: 
