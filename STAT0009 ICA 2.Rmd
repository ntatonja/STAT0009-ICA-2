...
We can use Sequential Monte Carlo methods and Sequential Importance sampling with resampling to respond the problem of weight degeneracy. Thanks to resampling, the particles with low weights are removed and those with high weights are replicated - we achieve the set of uniformly weighted particles that are approx. distributed according to the posterior $p(\phi_{0:t}|y_{1:t})$. Sequentially, at the new time points, the new hidden states are added from a conditional importance sampling -  $q_{t+1}(\phi_{t+1}^{(i)}| \phi_{0:t}^{(i)})$. A generic particle filter algorithm is as follows: firstly for samples $i=1,2,...,N$ we have $\tilde{\phi}_{0}^{(i)} \sim q\left(\phi_{0}\right)$ and we use that to calculate the normalised importance weights $\tilde{W}_{0}^{(i)} \propto \frac{p\left(\phi_{0}^{(i)}\right)}{q\left(\tilde{\phi}_{0}^{(i)}\right)}$, where $\sum_{i=1}^{N} \tilde{W}_{t}^{(i)}=1 .$ For every sample $i=1,2,...,N$  in $t$ the hidden states are: $\phi_{t}^{(i)} \sim q_{t}\left(\phi_{t} \mid \tilde{\phi}_{0: t-1}^{(i)}\right)$ and we add new dimension to the particles $\phi_{0: t}^{(i)}= \left(\tilde{\phi}_{0: t-1}^{(i)}, \phi_{t}^{(i)}\right)$. 
We normalise weights $W_{t}^{(i)} \propto \frac{p\left(y_{t} \mid \phi_{t}^{(i)}\right) p\left(\phi_{t}^{(i)} \mid \phi_{t-1}^{(i)}\right)}{q_{t}\left(\phi_{t}^{(i)} \mid \phi_{0: t-1}^{(i)}\right)} \tilde{W}_{t-1}^{(i)}$ where $\sum_{i=1}^{N} w_{t}^{(i)}=1$. We calculate the estimate:
$E_{t}^{\mathrm{PFn}}=\sum_{i=1}^{N} f\left(\phi_{0: t}^{(i)}\right) W_{t}^{(i)}.$ If $N_{\text {eff }} \leq c N$, we resample $\left\{\tilde{\phi}_{0: t}^{(i)}\right\}$ with replacement from $\left\{\phi_{0: t}^{(i)}\right\}$ with the normalized weights $W_{t}^{(i)}$ and  $\tilde{w}_{t}^{(1)}=1 / N$ to achieve a set of equally weighted particles $\left\{\tilde{\phi}_{t}^{(i)}, \tilde{W}^{(i)}=1 / N\right\} ;$ else set $\left\{\tilde{\phi}_{0: t}^{(i)}, \tilde{W}_{t}^{(i)}\right\}=\left\{\phi_{0: t}^{(i)}, W_{t}^{(i)}\right\}$. 

Resampling is associated with the additional variance in estimatates. If $c=1$, then we establish resampling at each time point. However, when we don't have much problem with degeneracy of importance weights, we might set $c=0.5$. In this case, we rasample only if we encounter sufficient evidence for weight degeneracy. We get two particle approximations at each time point- before resampling and after resampling; both of them are unbiased, but after resampling has generally lower variance. In this general particle filter we obtain a weighted sample of state sequences $\phi_{0: t}^{(i)}=\left(\phi_{0}^{(i)}, \phi_{1}^{(i)}, \ldots, \phi_{t}^{(i)}\right)$ that approximates a posterior over
state sequences $p\left(\phi_{0: t} \mid y_{1: t}\right)$.
Combining our calculations we get the estimator $E_{t}^{\mathrm{PFn}}$ over these sequences: $E^{\mathrm{PFn}}=\sum_{i=1}^{n} W_{t}^{(i)} f\left(\phi_{t}^{(i)}\right)$.

In the literature, we may find several variations of generic particle filter algorithm, e.g. the bootstrap filter and the auxilliary particle filter. Furthermore, it was proved that generally, Sequential Monte Carlo algorithms (and generic particle filter as one of them) satisfy Central Limit Theorem in a sense that as the number of particles grow to infinity, Sequential Monte Carlo estimates are normally distributed with mean around the true value.
