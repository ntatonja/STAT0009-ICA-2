...
# Load in a necessary library and set the seed, for reproducability.
library(gamlss.dist)
set.seed(123)

# Take a random sample from the exgaussian distribution and estimate the probability of the samples being above 3.
basic <- rexGAUS(2000, 0.4, 0.1, 0.5)
mean(basic>3)

# Compare the mean to the true value
true <- 1-pexGAUS(3, 0.4, 0.1, 0.5)
1-mean(basic>3)/true
# Estimated value is around 16% lower than true value

# Replicate samples for a later graph
basics <- rep(0,100)
for (i in 1:100) {
...
# Replicate samples for the next graph
expiss <- rep(0,100)
for (i in 1:100) {
  eis <- ISexp(2000)
  expiss[i] <- mean(eis$w)
