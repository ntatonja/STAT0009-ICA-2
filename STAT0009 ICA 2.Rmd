...
true <- 1-pexGAUS(3, 0.4, 0.1, 0.5) #true value
1-mean(basic>3)/true #calculating the percentage of how much lower the "basic"
                     #estimate is compared to the true value
...
Finally, the most popular resampling scheme, systematic resampling is based on the same idea as stratified resampling, however, it reduces the Monte Carlo variance further by using a single random number instead of different ones, to sample from each stratum. To understand this algorithm, first we define $\{\theta_t, W^{(i)}_t\}$ as the set of particles before resampling and $\{\tilde{\theta}_t, \tilde{W}^{(i)}_t\}$ as the set of particles after resampling. Then, the algorithm is as follows: first, draw $u$, a single random number from a uniform distribution between 0 and $1/N$. Then, define a random variable $U^i = (i − 1)/N + u$, for all $i = 1, \dots, N$. Next, find $r$ that satisfies $\sum^{r−1}_{k=1} W^{(k)} \leq U^i < \sum^{r}_{k=1} W^{(k)_t}$ and set $j(i)=r$, for all $i = 1, \dots, N$. Finally, for all $i$, set $\tilde{\theta}^{(i)}_t= \theta^{(j(i))}_t$ and $\tilde{W}^{(i)}_t = 1/N$. While systematic resampling is simple to implement and usualy performs well, contrary to residual and stratified resampling, it is not guaranteed to outperform multinomial resampling.
...
plot(1:Tp, y, col = "blue", pch = 20, xlab = "Time Points", ylab = "Values",
...
The bootstrap filter does well at sequentially estimating the new latent states at each time point, despite the relativly high variability shown by the observed states.
